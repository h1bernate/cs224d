...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sentIndicies = [tokens[s] for s in sentence]
    allWords = wordVectors[sentIndicies]
    sentVector = np.mean(allWords, axis=0)
    ### END YOUR CODE
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    # features: N x d, weights: d x C [num_classes]
    ...
    # calculate (\hat{y} - y)
    dJdt = np.zeros(prob.shape)
    dJdt[range(N), labels] = 1
    dJdt = prob - dJdt
    # features: N x d, dJdt: N x C
    grad = features.T.dot(dJdt) / N
