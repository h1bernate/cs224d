...
class Config(object):
  ...
  # hps are {embed_size, hidden_size, l2, window+size, dropout}
...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      L = tf.get_variable("L", shape=(len(self.wv), embed_size))
      embeddings = tf.nn.embedding_lookup(L, self.input_placeholder)
      window = tf.reshape(embeddings, (-1, self.config.window_size*embed_size), name="window")
      ### END YOUR CODE
      return window
