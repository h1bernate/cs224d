        ...
        while step < len(self.train_data):
            with tf.Graph().as_default(), tf.Session() as sess:
                ...
                if new_model:
                    ...
                    saver.restore(sess, './weights_lr/%s.temp'%self.config.model_name)
                for _ in xrange(RESET_AFTER):
                    ...
                    step+=1
                saver = tf.train.Saver()
                if not os.path.exists("./weights_lr"):
                    os.makedirs("./weights_lr")
                saver.save(sess, './weights_lr/%s.temp'%self.config.model_name, write_meta_graph=False)
        train_preds, _ = self.predict(self.train_data, './weights_lr/%s.temp'%self.config.model_name)
        val_preds, val_losses = self.predict(self.dev_data, './weights_lr/%s.temp'%self.config.model_name, get_loss=True)
    ...
    def train(self, verbose=True):
        ...
        for epoch in xrange(self.config.max_epochs):
            ...
            if val_loss < best_val_loss:
                 shutil.copyfile('./weights_lr/%s.temp'%self.config.model_name, './weights_lr/%s'%self.config.model_name)
...
def test_RNN():
    ...
    print 'Test'
    print '=-=-='
    predictions, _ = model.predict(model.test_data, './weights_lr/%s'%model.config.model_name)
